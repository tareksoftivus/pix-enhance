...
<div x-data="{ showRecovery: false }">
    <h2 class="text-3xl font-black tracking-tight text-neutral-950">{{ __('Two-Factor Authentication') }}</h2>
    <p class="mt-2 mb-8 text-sm leading-6 text-neutral-500" x-show="!showRecovery">{{ __('Enter the 6-digit code from your authenticator app to continue.') }}</p>
    <p class="mt-2 mb-8 text-sm leading-6 text-neutral-500" x-show="showRecovery" x-cloak>{{ __('Enter one of your recovery codes to continue.') }}</p>

    {{-- OTP Code Form --}}
    <form method="POST" action="{{ route('admin.two-factor.verify') }}" class="space-y-5" x-show="!showRecovery">
        @csrf
    ...
    {{-- Recovery Code Form --}}
    <form method="POST" action="{{ route('admin.two-factor.verify-recovery') }}" class="space-y-5" x-show="showRecovery" x-cloak>
        @csrf
